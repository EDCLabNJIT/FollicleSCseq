...
gg1 <- netVisual_circle(cellchat@net$CTRL$count, weight.scale = T, label.edge= F, title.name = "Number of interactions - CTRL", margin = 0, vertex.label.cex = .Machine$double.xmin)

gg2 <- netVisual_circle(cellchat@net$MM$count, weight.scale = T, label.edge= F, title.name = "Number of interactions - MM", margin = 0, vertex.label.cex = .Machine$double.xmin)
dev.off()
...
group.cellType <- c("Endothelium", "Immune", "Epithelium", rep("Granulosa",5), rep("Mesenchyme",6))
...
# coarse celltype
group.cellType <- c("Endothelium", "Immune", "Epithelium", rep("Granulosa",5), rep("Mesenchyme",6)) %>% as.factor
...
for (i in 1:length(object.list)) {
  netVisual_circle(object.list[[i]]@net$count.merged, weight.scale = T, label.edge= T, edge.weight.max = weight.max[3], edge.width.max = 12, arrow.size = .5,title.name = paste0("Number of interactions - ", names(object.list)[i]), margin = 0, )
}
dev.off()
...
par(mfrow = c(1,2), xpd=TRUE)
for (i in 1:length(object.list)) {
  netVisual_circle(object.list[[i]]@net$count.merged, weight.scale = T, label.edge= F, vertex.label.cex = .Machine$double.xmin, edge.weight.max = weight.max[3], edge.width.max = 12, arrow.size = .5,title.name = paste0("Number of interactions - ", names(object.list)[i]), margin = 0, )
}
dev.off()
}